"""
gen_v9.py — training data for the fan_center adapter.
...
Usage:
    python gen_v9.py                 # writes train_v9.jsonl + prints diagnostics
    python gen_v9.py --n 2000        # more samples
"""
...
def tilt_from_profile(rsrp):
    """
    The TOOL'S tilt, computed from the same five values the model is shown.

    This is deliberately NOT atan(h / true_range). The model cannot recover the crowd's
    real distance from an RSRP profile - range and angular spread act on it the same way,
    so the same five numbers can come from a near tight crowd or a far spread one. Asking
    for the true geometry would be asking it to learn noise, which is the mistake that
    produced two earlier adapters.

    Labelling with the tool's output makes the task learnable: read this profile, produce
    the number the arithmetic produces. Mirror of loop.js exactly.
    """
    # top two beams: peak plus its strongest ADJACENT neighbour, summed in linear power.
    # Conserves energy across a beam boundary so the estimate does not ripple as the
    # crowd crosses one.
    i = rsrp.index(max(rsrp))
    nb = [j for j in (i - 1, i + 1) if 0 <= j < len(rsrp)]
    j = max(nb, key=lambda k: rsrp[k]) if nb else i
    top2 = 10 * math.log10(10 ** (rsrp[i] / 10) + 10 ** (rsrp[j] / 10))

    # RSRP is already the mean per UE, so no division by N.
    pl = P_TX_DBM + G_MAX_DBI - top2
    d3d = 10 ** ((pl - 32.4 - 20 * math.log10(FC_GHZ)) / 21)
    raw = math.sqrt(max(0.0, d3d ** 2 - TOWER_H ** 2))

    # bias correction, refitted for mean-per-UE input over 30-200 m and 0-15 deg off-axis
    corrected = 0.000501 * raw * raw + 1.2224 * raw + 9.31
    rng_m = max(20.0, min(250.0, corrected))
    return round(math.degrees(math.atan2(TOWER_H, rng_m)), 1)
...
def make_sample(rng, n_ue=60):
    ...
    # Range is sampled UNIFORMLY IN TILT, not uniformly in metres.
    #
    # tilt = atan(h / r) is strongly non-linear, so a uniform range gives a badly
    # skewed LABEL distribution. Measured over 20k draws of uniform(30, 200):
    #
    #     3-10 deg   34%      <- a third of every label in one narrow band
    #    10-15 deg   29%
    #    15-20 deg   14%
    #    20-25 deg    9%
    #    25-30 deg    6%
    #    30-40 deg    8%      <- the near field, almost absent
    #
    # Two thirds of the labels sat under 15 deg. Only 18% of samples were closer
    # than 60 m. In a measured 121-tick run the crowd was inside 60 m for 32% of
    # ticks and that bucket carried the worst azimuth error (3.91 deg mean against
    # 2.38 deg at mid range). Under-trained where it is most used.
    #
    # Sampling uniformly in tilt covers the label space evenly and lifts near-field
    # coverage from 18% to 52%. The endpoints are the same 30-200 m span as before,
    # expressed as the angles they correspond to at TOWER_H.
    #
    # The earlier comment below still holds: range must vary CONTINUOUSLY. With three
    # fixed ranges the label took only 62 distinct values across 1500 samples, 55% of
    # them in one 5-degree band. Mean-per-UE aggregation removed the saturation, so
    # the whole span is usable.
    tilt_deg = rng.uniform(
        math.degrees(math.atan2(TOWER_H, 200.0)),   # 7.13 deg, the 200 m end
        math.degrees(math.atan2(TOWER_H,  30.0)),   # 39.81 deg, the 30 m end
    )
    crowd_r  = TOWER_H / math.tan(math.radians(tilt_deg))
    ...
    tilt_label = max(3.0, min(45.0, tilt_from_profile(rsrp)))
...
def main():
    ...
    ap.add_argument("--out", default="train_v9.jsonl")
    a = ap.parse_args()
