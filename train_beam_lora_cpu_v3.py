...
def make_synthetic(n):
    """(crowd position x current beam position) grid, with deliberate corner emphasis."""
    ticks = []
    grid = [-40, -30, -20, -10, 0, 10, 20, 30, 40]
    ...
    # 2. hard corners: crowd at one extreme, beam at the other
    ...
    # 3. tight crowds at the edges -> learn to commit to a full swing
    for _ in range(8):
        for crowd in (-40, 40):
            for cur in (-40, -20, 0, 20, 40):
                ticks.append(make_tick(crowd, cur, spread_deg=4.0))

    # 4. fill with random pairs, varied spread and range
    while len(ticks) < n:
