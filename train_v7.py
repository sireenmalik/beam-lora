"""
train_v7.py — LoRA fine-tune for the fan_center adapter.

Reads train_v7.jsonl produced by gen_v7.py. It does NOT generate data itself:
...
Run:
    python gen_v7.py --n 1500      # first, and read the diagnostics
...
if not os.path.exists(DATA):
    raise SystemExit(f"{DATA} not found. Run:  python gen_v7.py --n 1500")
...
for r in rows[:6]:
    ...
    tgt = json.loads(r["messages"][2]["content"])["fan_center"]
    got = generate(r["messages"])
    print(f"  target {tgt:+7.1f}  ->  {got[:70]}")
