...
def rsrp_per_beam(ue_azimuths, ue_ranges, fan_center, shadow=True):
    """
    ...
    lin = [0.0] * N_BEAMS
    for az, rng in zip(ue_azimuths, ue_ranges):
        d3d = math.hypot(rng, TOWER_H)
        pl = path_loss_38901(d3d)
        sf = random.gauss(0, SF_SIGMA_DB) if shadow else 0.0
...
def make_tick(crowd_center, current_fan, spread_deg=6.0, rng_m=100.0, n_ue=60):
    """
    One training tick. A crowd sits at crowd_center, the beam is currently at
    current_fan. The label is the RSRP-weighted centroid — the same quantity the
    pre-loop harness computes at inference.
    """
    az = [random.gauss(crowd_center, spread_deg) for _ in range(n_ue)]
    rg = [max(20.0, random.gauss(rng_m, rng_m * 0.12)) for _ in range(n_ue)]

    rsrp = rsrp_per_beam(az, rg, current_fan)

    # LABEL = the TRUE crowd bearing, not the centroid estimate.
    #
    # The centroid is the model's *observation*; it saturates at the fan edges and
    # collapses to the current position when the crowd is outside the fan entirely.
    # Training on the centroid would teach "crowd far right, beam far left -> stay put",
    # which is the exact paralysis we are trying to fix. Training on the true bearing
    # teaches the model to read a lopsided profile and commit to a full swing toward it.
    fan = int(round(max(-49, min(49, crowd_center))))

    # tilt follows the crowd's range: tilt = atan(h / range)
    ...
    # A profile that is flat at the floor carries no direction. Nudge the beam a little
    # toward the crowd so at least one edge beam sees it, mirroring what the live loop
    # does over successive ticks (it steps toward the crowd and re-measures).
    if max(rsrp) - min(rsrp) < 3:
        current_fan = current_fan + math.copysign(min(25, abs(crowd_center - current_fan)),
                                                  crowd_center - current_fan)
        rsrp = rsrp_per_beam(az, rg, current_fan)

    return {
        "rsrp": rsrp,
        "beam_az": [round(a, 1) for a in fan_azimuths(current_fan)],
        "current_fan_center": int(round(current_fan)),
        "fan_center": fan,
        "tilt": tilt,
        "action": "follow",
        "reason": reason,
...
def make_synthetic(n):
    """
    BALANCED sampling (the v3 -> v4 fix).

    v3 over-weighted the extremes: 48 corner examples plus 80 edge-crowd examples on
    top of an 81-cell grid, so roughly a third of the set was crowds at +/-40. The
    adapter learned "big answers are usually right" and then produced inverted or
    wildly overshooting angles on ordinary mid-range cases, which are the common case
    in the live demo.

    v4 keeps corner coverage (those cases still matter) but at a proportion that does
    not dominate, and adds explicit mid-range coverage on a finer grid.
    """
    ticks = []

    # 1. Fine grid over the positions the demo actually spends its time in.
    #    5 degree steps in the middle, coarser at the edges.
    fine = [-40, -32, -25, -20, -15, -10, -5, 0, 5, 10, 15, 20, 25, 32, 40]
    for crowd in fine:
        for cur in fine:
            ticks.append(make_tick(crowd, cur))

    # 2. Corner cases: kept, but 2x not 6x.
    ...
    # 3. Edge crowds: kept, but 2x not 8x.
    for _ in range(2):
        for crowd in (-40, 40):
            for cur in (-40, -20, 0, 20, 40):
                ticks.append(make_tick(crowd, cur, spread_deg=4.0))

    # 4. TRACKING MOVES - the steady state, and the bulk of the training set.
    #
    #    MEASURED from the running demo: a crowd walking at ~3 mph at ~100 m moves the
    #    centroid about 0.9 deg per tick, 2 deg at most. That is the movement the loop
    #    actually has to follow, tick after tick. v3 trained almost entirely on 10-80 deg
    #    swings, so it had never seen the scale it operates at and reached for a big
    #    answer every time. These deltas match the real crowd speed.
    for _ in range(6):
        for cur in fine:
            for delta in (-2.0, -1.5, -1.0, -0.5, 0.0, 0.5, 1.0, 1.5, 2.0):
                crowd = max(-42, min(42, cur + delta))
                ticks.append(make_tick(crowd, cur))

    # 4b. Slightly larger corrections: recovering after a clamp, or a brisk crowd.
    for _ in range(2):
        for cur in fine:
            for delta in (-6, -4, 4, 6):
                crowd = max(-42, min(42, cur + delta))
                ticks.append(make_tick(crowd, cur))

    # 5. Fill with random pairs.
    while len(ticks) < n:
        ticks.append(make_tick(
            random.uniform(-42, 42), random.uniform(-45, 45),
...
print("\n--- spot checks: does it produce a RANGE of angles, not just +/-29? ---")
for crowd, cur in [(40, -40), (-40, 40), (40, 0), (-40, 0), (15, 0), (-15, 0), (0, 20)]:
